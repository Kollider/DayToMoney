...
class SpendingForm(FlaskForm):
	...
	quantity_type = SelectField('Quantity type', choices=['л','мл','кг','г','шт'])#StringField('Quantity type', validators=[DataRequired()])
...
class MonthTypeForm(FlaskForm):
	name_of_type=StringField('Name of type', validators=[DataRequired()])
	amount_in_percent = FloatField('Percents')
	amount_in_money=FloatField('Money')
